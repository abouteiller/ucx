...
static void UCS_CLASS_DELETE_FUNC_NAME(uct_tcp_iface_t)(uct_iface_t*);

static ucs_status_t uct_tcp_iface_get_address(uct_iface_h tl_iface,
                                               struct sockaddr *iface_addr)
{
    uct_sockaddr_process_t *tcp_addr = (uct_sockaddr_process_t*)iface_addr;

    tcp_addr->sp_family = UCT_AF_PROCESS;
    tcp_addr->node_guid = ucs_machine_guid();
    tcp_addr->id        = 0;
    return UCS_OK;
}

static int uct_tcp_iface_is_reachable(uct_iface_h iface,
                                       const struct sockaddr* addr)
{
    return 0;
}

static ucs_status_t uct_tcp_iface_query(uct_iface_h iface,
...
static uct_iface_ops_t uct_tcp_iface_ops = {
    ...
    .ep_put_short        = uct_tcp_ep_put_short,
    .ep_am_short         = uct_tcp_ep_am_short,
};

static UCS_CLASS_INIT_FUNC(uct_tcp_iface_t, uct_pd_h pd, uct_worker_h worker,
                           const char *dev_name, size_t rx_headroom,
                           const uct_iface_config_t *tl_config)
{
    UCS_CLASS_CALL_SUPER_INIT(uct_base_iface_t, &uct_tcp_iface_ops, pd, worker,
                              tl_config UCS_STATS_ARG(NULL));

    if (strcmp(dev_name, UCT_TCP_DEV_NAME) != 0) {
...
UCS_CLASS_DEFINE_NEW_FUNC(uct_tcp_iface_t, uct_iface_t, uct_pd_h, uct_worker_h,
                          const char*, size_t, const uct_iface_config_t *);
static UCS_CLASS_DEFINE_DELETE_FUNC(uct_tcp_iface_t, uct_iface_t);


static ucs_status_t uct_tcp_query_tl_resources(uct_pd_h pd,
                                                ...
                                                unsigned *num_resources_p)
{
    ...
    ucs_snprintf_zero(resource->dev_name, sizeof(resource->dev_name), "%s",
                      UCT_TCP_DEV_NAME);
    resource->latency    = 25*1000; /* FIXME temp value */
    resource->bandwidth  = (long) (125 * pow(1024,2)); /* FIXME temp value */
...
UCT_TL_COMPONENT_DEFINE(uct_tcp_tl,
                        uct_tcp_query_tl_resources,
                        uct_tcp_iface_t,
                        UCT_TCP_TL_NAME,
                        "TCP_",
                        uct_tcp_iface_config_table,
                        uct_tcp_iface_config_t);
UCT_PD_REGISTER_TL(&uct_tcp_pd, &uct_tcp_tl);
